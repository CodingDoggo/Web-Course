...
<body>
    ...
    <script>
        ...
        async function showEditModal(id){
            ...
            let response = await fetch("http://localhost/PrviDomaci/api/user.php?id="+id);
            ...
            if(responseJSON.status == true){
                ...
                document.getElementById("country_drop_menu_edit").value = userData.country;
               //document.getElementById("cities_drop_menu_edit").value = userData.city; Ovaj dio ne radi jer ne stigne fetch valjda?
            }
            editModal.show();
        }
...
<script>
     async function cities_change(value,id){
          const cities_file = await fetch("http://127.0.0.1/PrviDomaci/cities.txt").then(res=>res.json());
          const countries_file = await fetch("http://127.0.0.1/PrviDomaci/countries.txt").then(res => res.json());  

            const element = countries_file.find(i=>i.name === value);

            console.log(element);
            console.log(cities_file);
            let stringToShow = "";

           for(let i=0;i<cities_file.length;i++){
             if(cities_file[i].country_id == element.id){
                stringToShow += `<option value="${cities_file[i].name}">${cities_file[i].name}</option>`;
             }
           }
           document.getElementById(id).innerHTML = stringToShow;
         }
 </script>